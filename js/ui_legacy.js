 ...
 */
let _safeBackdropInitializedLegacy = false;

export function initSafeModalBackdropDismiss() {
  if (typeof window === 'undefined' || typeof document === 'undefined') return;
  if (_safeBackdropInitializedLegacy || window._safeBackdropDismissInitialized) return;
  _safeBackdropInitializedLegacy = true;
    ...
    <div style="display:flex;flex-direction:column;gap:20px;">
      ${depts.map(dept => {
        ...
        const courseRows = curriculum.map(course => {
          ...
          // Real-time Check: Şu anki kadro ile en iyi kim verebilir?
          const potential = deptFaculty.reduce((best, f) => {
            const specs = f.specializations || [];
            const req   = (course.requiredExpertise || '').toLowerCase();
            let q = 1; // Temel eşleşme
            if (req && specs.some(s => s.toLowerCase() === req)) q = 2;
            else if (req && specs.some(s => s.toLowerCase().includes(req.substring(0,5)) || req.includes(s.toLowerCase().substring(0,5)))) q = 1;

            if (q > best.q) return { q, f };
            if (q === best.q && (f.stats?.teaching || 50) > (best.f?.stats?.teaching || 50)) return { q, f };
            return best;
          }, { q: 0, f: null });
...
export function renderFacultyPanel(state, onTransferMarket, onFacultyDetail, onOpenPosition) {
  ...
  // Görünüm toggle (üst seviye değişkenleri kullan)
...
export function renderBolumlerPanel(state, onAssignHead, onReassignFaculty) {
  ...
  // 1. Bilinen tüm fakülte tanımları (FACULTIES ve state.fakulteler birleşimi)
  const allFaculties = {};
  ...
  // 2. Aktif bölümleri fakültelerine göre grupla
  const fakGroups = [];
  const assignedDeptIds = new Set();

  for (const [fId, fData] of Object.entries(allFaculties)) {
    const activeDepts = depts.filter(d => (fData.departments || []).includes(d.id) || d.faculty === fId);
  ...
  // 3. Hiçbir fakülteye girmemiş kalan bölümler (Fail-safe: ASLA kaybolamaz!)
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:12px;margin-bottom:24px;">
        ${buildings.map(b => {
          ...
          if (b.type === 'fakulte_binasi' || b.type === 'amfi') {
            ...
            const totalSporCap = ((state.buildings || []).filter(bld => bld.type === 'spor_tesisi' && bld.isCompleted)
              .reduce((s, bld) => s + ((bld.currentCapacity?.dailyUsers) || 500), 0));
            ...
            const pct         = Math.round((totalSporCap / sporDemand) * 100);
            const sfxIcon     = pct >= 100 ? '✅' : pct >= 75 ? '⚠️' : '❌';
            const sfxColor    = pct >= 100 ? '#4ade80' : pct >= 75 ? '#f59e0b' : '#ef4444';
            const sfxText     = pct >= 100 ? 'Yeterli' : pct >= 75 ? 'Sınırda' : 'Yetersiz';
            detailsHtml = `
              <div style="margin-bottom:6px;">
                <div style="font-size:11px;font-weight:600;color:#94a3b8;margin-bottom:3px;">⚽ HİZMET KAPASİTESİ</div>
                <div style="font-size:11px;color:#cbd5e1;">• Tesis: ${sporCap.toLocaleString('tr-TR')} · Kampüs Toplam: ${totalSporCap.toLocaleString('tr-TR')}</div>
              ...
              <div style="margin-bottom:6px;">
                ...
                ${pct < 75 ? `<div style="font-size:11px;color:#ef4444;">• Kapasite yetersiz — öğrenci şikâyeti artıyor</div>` : ''}
              </div>`;
...
export function renderResearchPanel(state, onResearchBudget, onProjectDecision) {
      ...
      <div>
        ...
        <div id="active-projects-container" style="${window._activeProjectsCollapsed ? 'display:none;' : ''}">
          <div style="font-size:11px;color:var(--text-muted);margin-bottom:10px;padding:0 4px;">
            Toplam Bütçe: ${formatMoney(totalProjectBudget)} — Üni Payı: ${formatMoney(uniShareTotal)}/dönem
          </div>
          <div style="display:flex;flex-direction:column;gap:8px;">
  ...
  // v0.4.52: Proje başvurularını katla/aç
  const toggleAppsBtn = el('toggle-research-apps');
  if (toggleAppsBtn) {
    on(toggleAppsBtn, 'click', () => {
      window._researchAppsCollapsed = !window._researchAppsCollapsed;
      renderResearchPanel(state, onResearchBudget, onProjectDecision);
    });
  }

  // v0.4.53: Aktif projeleri katla/aç
  const toggleActiveBtn = el('toggle-active-projects');
  if (toggleActiveBtn) {
    on(toggleActiveBtn, 'click', () => {
 ...
 */
let _lastLegacyNotification = { message: '', time: 0 };
...
export function showNotification(message, type = 'info', duration = 3500) {
  ...
  if (_lastLegacyNotification.message === trimmed && (now - _lastLegacyNotification.time) < 1200) {
    return;
  }
  const existing = [...container.querySelectorAll('.toast-content, .notification-text')].find(c => c.textContent.trim() === trimmed);
  if (existing) {
    return;
  }

  _lastLegacyNotification = { message: trimmed, time: now };
